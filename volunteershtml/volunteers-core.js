(function(){'use strict';
 ...
 function saveSession(v,y){const s=school();const session={role:'volunteer',volunteerId:v.volunteerId,displayName:v.displayName,schoolId:s.id,schoolName:v.schoolName||s.name||s.id,year:String(y),accessLevel:Number(v.accessLevel||1),status:v.status};localStorage.setItem('role','volunteer');sessionStorage.setItem(SESSION_KEY,JSON.stringify(session));localStorage.setItem(SESSION_KEY,JSON.stringify(session));sessionStorage.removeItem('somap.volunteerWelcomeDownloaded');return session}
