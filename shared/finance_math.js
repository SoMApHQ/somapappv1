...
(function(global){
  ...
  function parseMonthlyAmountFromText(...values) {
    for (const value of values) {
      ...
      if (Number.isFinite(amount) && amount > 0) return amount < 1000 ? Math.round(amount * 1000) : Math.round(amount);
    }
    return 0;
  }
  function buildMonthlyScheduleFromRule(plan = {}, planId = '') {
    const rule = plan.monthly && typeof plan.monthly === 'object' ? plan.monthly : {};
    const monthlyAmount = Number(rule.amount || rule.monthlyAmount || 0) ||
      parseMonthlyAmountFromText(planId, plan.id, plan.planId, plan.name, plan.title, plan.label);
    ...
    return monthLabels.flatMap((label, idx) => {
      ...
      return [{
        ...
        amount: monthlyAmount * (doubleMonths.includes(month) ? 2 : 1),
        weight: 1,
      }];
    });
  }
  ...
  function schoolPrefix(){
    const schoolId =
      ...
      (global.SOMAP && (typeof global.SOMAP.getSchoolId === 'function'
  ...
  function computeStudentFinancials(student, targetYear){
    ...
    const debtForCollection = isOverdueDebt
      ? debtTillNowCapped
      : (extensionActive
          ? Math.max(0, hasCarryOutstanding ? carryOutstanding : 0)
          : Math.max(0, (hasCarryOutstanding ? carryOutstanding : 0) + overdueBaseOutstanding));
    const rawDebtForDisplay = debtForCollection > 0
      ? debtForCollection
      : (!extensionActive && effectivelyCarryDebt && yearBalance > 0 ? yearBalance : 0);
...
function buildFinanceStudents(
  ...
  year = SOMAP_DEFAULT_YEAR
    ...
    const toMs = (value) => {
      ...
      if (!s) return null;
      const auto = s.match(/^AUTO-(\d{10,13})$/i);
      if (auto) {
    ...
    const getRegistrationMs = (stu) => {
      ...
      return (
        ...
        toMs(stu.id) ??
        toMs(stu.studentId) ??
        toMs(stu.admissionNumber) ??
        toMs(stu.admissionNo) ??
        null
      );
    };
    ...
    const legacyCutoffMs = Date.UTC(2026, 6, 1);
    ...
    ids.forEach((id) => {
      ...
      const resolvedPlanSchedule = isMonthlyPlan && resolvedPlan
        ? buildMonthlyScheduleFromRule(resolvedPlan, resolvedPlanId)
        : (Array.isArray(resolvedPlan?.schedule) ? resolvedPlan.schedule : []);
      ...
      let baseFeeCandidate =
        ...
        base.requiredFee ??
        0;
      if (isMonthlyPlan && (!baseFeeCandidate || baseFeeCandidate === 0) && resolvedPlanSchedule.length) {
        const totalFromSchedule = resolvedPlanSchedule.reduce((sum, row) => sum + (Number(row.amount) || 0), 0);
      ...
      } else if (resolvedPlanSchedule.length) {
        record._planSchedule = resolvedPlanSchedule;
      }
