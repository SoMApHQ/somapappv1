(function(global) {
  ...
  async function createDocument(audit) {
    ...
    function page(title) {doc.addPage();header();y=80;heading(title);}
    function heading(title) {doc.setFontSize(13);doc.text(title,margin,y);y+=22;}
    function table(head,body) {
      doc.autoTable({startY:y,head:[head],body:body.length?body:[['No records.']],showHead:'everyPage',
        ...
        alternateRowStyles:{fillColor:[245,248,252]},didDrawPage:header});
      y=doc.lastAutoTable.finalY+20;
    }
    function cards(title,rows) {
      page(title);
      if(!rows.length) {table(['Status'],[['No records.']]);return;}
      rows.forEach(t=>{if(y>height-180)page(title+' (continued)');
    ...
    page('Category totals');table(['Category','Amount'],Object.entries(audit.categoryTotals).map(([k,v])=>[k,money(v)]));
    const txns=audit.transactions || [], income=txns.filter(t=>t.moneyIn>0);
    cards('Complete incoming-payment ledger and student trace',income);
    ...
    page('Grouped withdrawal register and transfer charges');
